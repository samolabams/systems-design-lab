...
|---|---|---|
| [infra/gateway/nginx/nginx.conf](https://github.com/samolabams/systems-design-lab/blob/main/infra/gateway/nginx/nginx.conf) | Active Nginx gateway config | `resolver 127.0.0.11`, `proxy_pass`, timeout settings, and passive failover settings |
| [infra/gateway/nginx/variants/round-robin.conf](https://github.com/samolabams/systems-design-lab/blob/main/infra/gateway/nginx/variants/round-robin.conf) | Explicit round-robin variant | Static `upstream` block syntax |
| [infra/gateway/nginx/variants/least_conn.conf](https://github.com/samolabams/systems-design-lab/blob/main/infra/gateway/nginx/variants/least_conn.conf) | Least-connections variant | `least_conn` directive |
| [infra/gateway/nginx/variants/ip_hash.conf](https://github.com/samolabams/systems-design-lab/blob/main/infra/gateway/nginx/variants/ip_hash.conf) | Sticky routing variant | `ip_hash` directive |
| [infra/gateway/haproxy/haproxy.cfg](https://github.com/samolabams/systems-design-lab/blob/main/infra/gateway/haproxy/haproxy.cfg) | HAProxy contrast config | `balance roundrobin`, `option httpchk`, `server-template`, and the stats listener |
...
- [Scaling](/modules/scaling/) for vertical and horizontal growth.
- [Service discovery](/modules/service-discovery/) for finding changing backends.
- [API gateway](/modules/api-gateway/) for the public edge path.
