...
presigned URLs are visible. [Edge caching](/modules/edge-caching/)
shows how a CDN or edge cache can sit in front of object storage for delivery.
[Vector stores](/modules/vector-store/) solve a different problem: similarity
...
| CDN delivery layer | edge cache, described in [edge caching](/modules/edge-caching/) |
...
- Put [edge caching](/modules/edge-caching/) in front of object storage and
   reason about repeated downloads.
- Compare this module with [databases](/modules/databases/) and decide which
   fields belong in structured metadata versus blob storage.
- Use the [API design](/modules/api-design/) module to think through upload
