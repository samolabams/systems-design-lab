...
The DNS files live in [infra/dns/](https://github.com/samolabams/systems-design-lab/tree/main/infra/dns):

| File | Role in the lab | What to inspect |
|---|---|---|
| [Corefile.root](https://github.com/samolabams/systems-design-lab/blob/main/infra/dns/Corefile.root) | Configures `dns-root` as the root-like server for `.` | The `file` plugin points at `db.root` |
| [db.root](https://github.com/samolabams/systems-design-lab/blob/main/infra/dns/db.root) | Root zone data | The `NS` delegation and glue `A` record for `shop.internal.` |
| [Corefile.auth](https://github.com/samolabams/systems-design-lab/blob/main/infra/dns/Corefile.auth) | Configures `dns-auth` as authoritative for `shop.internal` | The `file` plugin serves the zone; `loadbalance` can rotate A-record order |
| [db.shop.internal](https://github.com/samolabams/systems-design-lab/blob/main/infra/dns/db.shop.internal) | Authoritative zone data | The actual records queried by the demo |
| [Corefile.resolver](https://github.com/samolabams/systems-design-lab/blob/main/infra/dns/Corefile.resolver) | Configures `dns-resolver` as the client-facing resolver | The `forward` plugin sends queries upstream; `cache 30` makes TTL countdown visible |
...
1. **Add a name.** In [infra/dns/db.shop.internal](https://github.com/samolabams/systems-design-lab/blob/main/infra/dns/db.shop.internal),
...
- [Load balancing](/modules/load-balancing/) for traffic distribution after name resolution.
- [Service discovery](/modules/service-discovery/) for internal service lookup.
- [Multi-region, DR and backups](/modules/multi-region-dr/) for failover thinking.
