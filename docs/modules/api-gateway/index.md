...
|---|---|---|
| [infra/gateway/nginx/nginx.conf](https://github.com/samolabams/systems-design-lab/blob/main/infra/gateway/nginx/nginx.conf) | Active gateway config | `listen 80`, `location`, `proxy_pass`, forwarded headers, timeouts |
| [apps/url-shortener/src/routes/index.js](https://github.com/samolabams/systems-design-lab/blob/main/apps/url-shortener/src/routes/index.js) | App route table | `/health`, `/metrics`, `/shorten`, `/jobs`, and `/:code` |
| [apps/url-shortener/src/controllers/linkController.js](https://github.com/samolabams/systems-design-lab/blob/main/apps/url-shortener/src/controllers/linkController.js) | App request handlers | `POST /shorten` and redirect behavior |
| [modules/load-balancing/README.md](/modules/load-balancing/) | Load balancing lesson | How gateway traffic is distributed across replicas |
| [modules/rate-limiting/README.md](/modules/rate-limiting/) | Rate limiting lesson | A gateway policy built on top of this entry point |
...
- [Load balancing](/modules/load-balancing/) for distributing gateway traffic across replicas.
- [Rate limiting](/modules/rate-limiting/) for adding boundary protection.
- [Circuit breakers](/modules/circuit-breakers/) for failure behavior behind the gateway.
