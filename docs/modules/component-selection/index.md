...
**Track:** Foundations
**Prerequisites:** [The design method](/modules/design-method/)
...
demonstration. In a real design, or in [design method](/modules/design-method/) step 4, the skill is
...
[design method](/modules/design-method/) step 1).
...
[database scaling](/modules/database-scaling/). Measured read pressure points
...
|---|---|---|---|
| Relational (Postgres/MySQL) | ACID, joins, strong consistency are required | vertical-scale ceiling; sharding is manual | [Databases](/modules/databases/) |
| Document (MongoDB) | schema varies per record, nested data, few joins | weak cross-document transactions | [Databases](/modules/databases/) |
| Key-value (Redis/DynamoDB) | point lookups by key at large scale | no rich queries | [caching](/modules/caching/) |
| Column-family (Cassandra) | write-heavy, time-series, multi-DC | eventual consistency; query-first modeling | [event streaming](/modules/event-streaming/) |
| Object store (S3) | large blobs: images, video, backups | not for low-latency mutable rows | [object storage](/modules/object-storage/) |
| Vector store / vector index | semantic retrieval, recommendations, similar-item search, image similarity | approximate results; embedding generation and refresh cost | [vector store](/modules/vector-store/) |
...
| Option | Reach for it when | Trade-off | Demo |
|---|---|---|---|
| Queue (RabbitMQ/SQS) | decouple work, competing consumers, ack-per-message | deletes on consume; no replay | [async queues](/modules/async-queues/) |
| Log (Kafka) | high throughput, replay, event sourcing, multiple readers | offsets and retention must be operated | [event streaming](/modules/event-streaming/) |
| Managed (SQS) | managed operations are required | fewer routing features | [async queues](/modules/async-queues/) |
...
| Option | Reach for it when | Trade-off | Demo |
|---|---|---|---|
| Redis | rich types (sorted sets, counters), persistence option | single-threaded; more memory overhead | [caching](/modules/caching/) |
| Memcached | simple key→blob caching | no data structures, no persistence | [caching](/modules/caching/) |
| CDN (Cloudflare/CloudFront) | static assets served near the user | only for cacheable, mostly-static content | [edge caching](/modules/edge-caching/) |
...
| Option | Reach for it when | Trade-off | Demo |
|---|---|---|---|
| L4 load balancer (HAProxy) | raw TCP throughput, simple routing | no awareness of HTTP semantics | [load balancing](/modules/load-balancing/) |
| L7 reverse proxy (Nginx) | path/header routing, TLS, caching | slightly more overhead per request | [load balancing](/modules/load-balancing/) |
| API gateway (Kong/Nginx+) | auth, rate-limit, aggregation across services | one more hop and component to scale | [API design](/modules/api-design/) |
...
Nothing to start. Apply the catalog to an [design method](/modules/design-method/)
...
- [When not to scale](/modules/when-not-to-scale/) for avoiding premature complexity.
- [Estimation](/modules/estimation/) for sizing the actual problem.
- [The design method](/modules/design-method/) for turning choices into a complete design.
