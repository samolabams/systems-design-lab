...
# (base, always-on) — the saga logic is the lesson, not the transport.
set -uo pipefail
...
comp_shipment() { # order_id
  ...
  if [ "$claimed" != "1" ]; then echo "    comp ship($oid): already applied — skip (idempotent)"; return; fi
...
comp_payment() { # order_id
  ...
  if [ "$claimed" != "1" ]; then echo "    comp charge($oid): already applied — skip (idempotent)"; return; fi
...
comp_inventory() { # order_id sku qty
  ...
  if [ "$claimed" != "1" ]; then echo "    comp reserve($oid): already applied — skip (idempotent)"; return; fi
