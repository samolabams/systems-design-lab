...
passing | sed 's/^/    /'
note "No Nginx config was edited â€” a discovery-aware balancer would pull this set"
note "from Consul and route only to it. New replicas appear here automatically."
pause

step "Stop one replica â€” watch Consul evict it" "the active health check fails, the instance drops from discovery"
...
passing | sed 's/^/    /'
note "$first_id is gone from the healthy set within a couple of check intervals â€”"
...
note "ever discoverable â€” the capability the static config could not provide."
