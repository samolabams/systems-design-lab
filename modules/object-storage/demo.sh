...
# wget from the app container â€” an UNPRIVILEGED client on the backend network,
...
step "Create a private bucket" "a flat namespace of buckets/keys â€” not a filesystem, not a table"
...
note "flat key namespace, byte sizes tracked by the store â€” the DB never sees these bytes"
pause
...
note "^ refused â€” objects are not world-readable by default"
pause
...
cat <<EOF
  ...
  A production system would also:
    â€¢ serve blobs through a CDN with MinIO as the ORIGIN  -> see edge caching (edge caching)
    â€¢ use multipart/resumable uploads for very large objects (mc does this transparently)
    â€¢ store object metadata, ownership, and permissions in the database
