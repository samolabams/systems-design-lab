...
step "Straight to the origin (gateway): the replica rotates" "host changes — every request hit the origin"
...
step "First request THROUGH the edge: a MISS" "X-Cache-Status: MISS — the edge had to ask the origin"
...
note "The host is frozen to whichever replica filled the cache — the origin is"
