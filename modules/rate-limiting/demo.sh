...
step "Flood the gateway" "first few 200, the rest 429 â€” the flood is shed at the edge"
...
note "An app-level limiter (per API key / user) can't use in-memory counters â€” with"
note "N replicas each would allow the full quota, so the real limit becomes NÃ—quota."
